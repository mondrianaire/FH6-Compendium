...
(function () {
  ...
  function buildLab() {
    ...
    // ---- LIVE · COURSE dashboard: a definitive split between LEARNING the course and TUNING FEEDBACK, each with its own status ----
    const liveCourseDashboard = (co, s) => {
      ...
      const known = (tr.sessions || 0) > 1 || (tr.laps || 0) > lapsN; const turnsN = tu.count || 0; const refsOwn = dr.own_refs || 0, refsPred = dr.predicted || 0;
      const learn = { identified: known, mapped: !!(co.geometry && (co.geometry.turns || []).length), turnsConf: tu.track_confidence || tu.confidence || 0, refs: turnsN ? Math.min(1, refsOwn / turnsN) : 0, profile: !!co.profile };
      const learnPct = Math.round(((learn.identified ? 1 : 0.5) * 0.15 + (learn.mapped ? 0.2 : 0) + learn.turnsConf * 0.35 + learn.refs * 0.3) * 100);
      const needRefs = Math.max(1, Math.ceil(turnsN * 0.5)); const feedbackReady = learn.mapped && refsOwn >= needRefs;
      const tiles = [[esc(p.rn), "course"], [`${lapsN} / ${tr.laps || lapsN}`, "laps · session / on record"], [co.best_lap ? co.best_lap.toFixed(3) : "—", "best lap · session"], [tr.best ? tr.best.best_lap.toFixed(3) : "—", "best lap · track"], [f.on && f.ev && f.lapt ? f.lapt.toFixed(1) + " s" : "—", `lap ${f.on && f.ev ? (f.lapn || 1) : "—"} · running`], [`${turnsN}${tu.possible ? " +" + tu.possible : ""}`, `turns · ${Math.round((tu.confidence || 0) * 100)}% earned`], [`${dr.on_reference || 0}/${dr.compared || 0}`, "turns on reference"], [`${learnPct}%`, "course learning"]];
      ...
      return `<div class="lab-tiles" style="margin-bottom:8px">${tiles.map(([v, l]) => `<div class="lab-tile"><b>${v}</b><span>${l}</span></div>`).join("")}</div>
        <div class="card-grid">
          <div class="lab-corner" style="border-left:4px solid var(--accent2)"><div class="card-row" style="margin-top:0"><strong>📚 Course learning — what we know about this track</strong><span class="chip" style="border-color:var(--accent2);color:var(--accent2);font-weight:700">${learnPct}%</span></div>
            <div class="lab-bar" style="height:6px;margin:6px 0"><i style="width:${learnPct}%;background:var(--accent2)"></i></div>
            <div style="display:flex;flex-wrap:wrap;gap:4px;font-size:11px;margin-bottom:6px">${st("identified", learn.identified, known ? `known track · ${tr.laps} laps on record over ${tr.sessions} sessions` : "new track — learning it from this session")}${st("mapped", learn.mapped, learn.mapped ? `${(co.geometry.turns || []).length} turns mapped from coordinates` : "the map appears after the first full lap")}${st("turn count", learn.turnsConf >= 0.7, `${Math.round(learn.turnsConf * 100)}% earned across laps`)}${st("references", learn.refs >= 0.5, `${refsOwn}/${turnsN} turns have a reference for THIS car${refsPred ? ` · ${refsPred} predicted from geometry × grip` : ""}`)}${st("profile", learn.profile, learn.profile ? "demands known" : "after a lap")}</div>
            ${p.track}${p.map}${p.turns}${p.profile}${p.laps}</div>
          <div class="lab-corner" style="border-left:4px solid ${feedbackReady ? "var(--accent)" : "var(--muted)"}"><div class="card-row" style="margin-top:0"><strong>🏋 Tuning feedback — this car on this track</strong><span class="chip" style="border-color:${feedbackReady ? "#00d27a" : "var(--warn,#e3b341)"};color:${feedbackReady ? "#00d27a" : "var(--warn,#e3b341)"};font-weight:700">${feedbackReady ? "ACTIVE" : "WARMING UP"}</span></div>
            <p class="why" style="font-size:11px;margin:4px 0 6px">${feedbackReady ? `references exist for ${refsOwn}/${turnsN} turns — the per-turn deltas and slider suggestions below are grounded in this car's own best passes` : `gated on learning: ${!learn.mapped ? "finish one full lap so the track is mapped" : `${refsOwn}/${turnsN} turns have a reference for this car — ${Math.max(0, needRefs - refsOwn)} more clean turn${needRefs - refsOwn === 1 ? "" : "s"} needed (geometry × grip predictions fill in meanwhile)`}`}</p>
    ...
    function courseSection(s, isLive) {
      ...
      if (isLive) return `${courses.length ? courses.map((co) => `<div class="block" style="border-color:var(--accent2)"><h3 style="margin-top:0">🏟 Course dashboard <span class="chip">updates every ~20 s of driving</span></h3>${selBar}${liveCourseDashboard(co, s)}</div>`).join("") : atlasPick && selModel ? `<div class="block" style="border-color:var(--accent)"><h3 style="margin-top:0">🏟 Course — from the track record</h3>${selBar}<div class="card-grid">${modelCourseCard(selModel)}</div></div>` : `<div class="block" style="border-color:var(--accent2)"><h3 style="margin-top:0">🏟 Course — nothing scoped yet</h3><p class="why" style="font-size:12px;margin:0">A course appears when you run a timed event (Rivals · race · time trial) or lap a marked reference loop — mark one with 📍 in the stream bar above and drive back through the start. Everything outside those windows is Free Tuning.</p></div>`}${eventsTable(s)}${routesAtlas(s)}`;
    ...
    // course MAP from coordinates: reference-lap path + latest lap overlaid + mapped turns (filled = loaded in telemetry, hollow = mapped but taken flat)
    const courseMap = (geo, corners) => {
      ...
      const driven = new Set((corners || []).map((k) => k.geo_id).filter(Boolean));
      return `<svg viewBox="0 0 ${W} ${H}" class="tz-svg" style="max-width:${W}px;background:var(--bg);border-radius:8px">
        ...
        ${(geo.turns || []).map((g) => { const dr = driven.has(g.id); return `<g><circle cx="${X(g.apex[0]).toFixed(1)}" cy="${Y(g.apex[1]).toFixed(1)}" r="5" fill="${dr ? "#e5414e" : "var(--bg)"}" stroke="${dr ? "#e5414e" : "var(--muted)"}" stroke-width="1.5"><title>${g.id} · ${g.dir} · r≈${g.radius_m} m · ${g.len_m} m long${dr ? "" : " · not loaded in telemetry (taken flat?)"}</title></circle><text x="${(X(g.apex[0]) + 6).toFixed(1)}" y="${(Y(g.apex[1]) + 3).toFixed(1)}" fill="var(--txt)" font-size="9" font-weight="700">${g.id.replace("G", "")}</text></g>`; }).join("")}
      </svg>`;
    };
    ...
    const mapCardHtml = (geo, corners) => (geo ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--bg2)">
          <div class="card-row" style="margin-top:0"><strong style="font-size:12px">🗺 Course map — learned from coordinates (${geo.length_m} m · ${(geo.turns || []).length} turns mapped${geo.driven != null ? ` · ${geo.driven} loaded in telemetry` : ""})</strong><span class="chip">ref lap ${(geo.ref_lap || {}).lap || "—"}${geo.last_lap ? ` · latest lap ${geo.last_lap.lap} overlaid` : ""}</span></div>
          <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-start">${courseMap(geo, corners)}<div style="font-size:10.5px;min-width:150px;max-width:320px"><div><span style="color:var(--accent2)">━</span> course path (reference lap) · <span style="color:var(--warn,#e3b341)">╌</span> your latest lap · <span style="color:var(--muted)">─</span> every recorded lap / attempt${(geo.layout_paths || []).length ? ` (${geo.layout_paths.length}${new Set(geo.layout_paths.map((l) => l.session)).size > 1 ? ", " + new Set(geo.layout_paths.map((l) => l.session)).size + " sessions" : ""}) — the spread of lines is the course layout` : ""} · <span style="color:#e5414e">●</span> turn loaded in telemetry · ○ mapped but not loaded</div><div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:3px">${(geo.turns || []).map((g) => `<span class="chip" title="${g.dir} · r≈${g.radius_m} m · ${g.len_m} m long · ${g.deg != null ? g.deg + "° of heading change" : ""}">${g.id} ${g.dir === "L" ? "⬅" : "➡"} r${g.radius_m}${g.deg != null ? " · " + g.deg + "°" : ""}</span>`).join("")}</div>${(geo.not_driven || []).length ? `<p class="why" style="font-size:10px;margin:5px 0 0">${geo.not_driven.join(", ")}: mapped turns your telemetry never loaded — taken flat or cut; they still count as turns of the course</p>` : ""}</div></div>
        </div>` : "");
    ...
    const modelCourseCard = (m) => {
      ...
      return `<div class="lab-corner" style="border-left:4px solid var(--accent)">
        ...
        ${trackRecordHtml(tr, rn)}${m.profile ? profileCardHtml(m.profile) : ""}${geo ? mapCardHtml(geo, []) : ""}
    ...
    const courseParts = (co, s) => {
      ...
      const turnsCard = tu ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--bg2)">
          <div class="card-row" style="margin-top:0"><strong style="font-size:12px">🔁 Turns — count earned lap by lap</strong><span class="chip" style="border-color:${TC};color:${TC};font-weight:700">${tu.count} turn${tu.count === 1 ? "" : "s"}${tu.possible ? ` + ${tu.possible} possible` : ""} · ${Math.round(tu.confidence * 100)}% · ${tu.laps} lap${tu.laps === 1 ? "" : "s"}</span></div>
...
${open.length ? `<div style="font-size:11px;color:var(--warn,#e3b341);margin-top:4px">🔍 ${open.map((a) => a.text).join(" · ")}</div>` : ""}</div>`; }).join("")}`;
      return { rn, cov, nl, tu, tr, header, track: trackCard, profile: profileCard, map: mapCardHtml(co.geometry, co.corners), turns: turnsCard, probes, laps, corners, driving, advice };
    };
