...
(function () {
  ...
  function buildLab() {
    ...
    const diskMatchBar = (r, ordinal) => {
      ...
      else if (m.how === "signature") status = `<div class="dm-ok">✓ matched to the car you${m.live ? "'re driving" : " last drove (held while parked)"} — ${m.live_cyl}-cyl${m.live_pi ? `, PI ${m.live_pi}` : ""}${ties >= 2 ? ` <span class="why" style="font-weight:400">· ${ties} builds share this signature — drive up through the gears to pin the exact one, or pick below</span>` : ""}</div>`;
