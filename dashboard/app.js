...
(function () {
  ...
  function buildLab() {
    ...
    const carLblHtml = (s, o, short) => { const c = car(s, o); if (!c) return esc(`#${o}`); const nm = carName(c); return `${esc(nm ? nm : "#" + c.ordinal)} ${piBadge(c.class, c.pi, true)}${short ? "" : ` ${esc(c.drivetrain)} ${c.cyl}cyl${esc(stintLbl(s, o))}`}`; };
    ...
    const courseIdRow = (cid) => {
      ...
      const thumb = lv && lv.thumb ? `<img class="tl-blvy" src="${liveUrl}/livery-thumb?d=${encodeURIComponent(lv.dir)}" alt="">` : "";
    ...
    const paintActiveCar = () => { const el = host.querySelector("#lvActiveCar"); if (!el) return; const f = live.frame; const on = f && f.on;
      ...
      el.innerHTML = `<div style="display:flex;align-items:center;gap:12px;padding:9px 14px;border:2px solid ${col};border-radius:10px;background:${changed ? "rgba(227,179,65,.10)" : on ? "rgba(0,210,122,.06)" : "var(--bg2)"};box-shadow:inset 6px 0 0 ${col}">
        ...
        <div style="flex:1;min-width:0"><div style="font-size:18px;font-weight:800;color:${col};line-height:1.1">${esc(nm)}</div><div class="why" style="font-size:12px">${on ? `${piBadge(f.cls, f.pi, true)} · ${f.drv} · ${f.cyl || ""}cyl${f.on && f.ev ? " · in an event" : ""}` : "parked — last active car"} · build ${String(cid).split("|").slice(0, 4).join("|")}</div></div>
    ...
    const diskMatchBar = (r, ordinal) => {
      ...
      const picker = (saves.length > 1 || pick) ? `<div class="dm-picker">${saves.map((s) => {
        ...
        return `<button class="dm-chip${on ? " on" : ""}" data-diskpick="${ordinal}|${s.ts}" title="${s.locked ? "downloaded" : "self-made"} · saved ${_tsFmt(s.ts)}">${s.pi != null ? piBadge(null, s.pi, true) : "PI ?"}${s.cyl != null ? ` · ${s.cyl}cyl` : ""} <span class="dm-date">${_tsFmt(s.ts)}</span></button>`;
    ...
    const cloneModeBanner = (locked, payload) => {
      if (locked) {
        ...
        return `<div class="clone-mode locked"><div class="cm-txt"><b>🎯 CLONE TARGET LOCKED</b> — ${esc(t.name || "#" + t.ordinal)}${sm.pi_total ? ` ${piBadge(null, sm.pi_total, true)}` : ""}<div class="why">Frozen — not decoding your live car. Build your replica to match this. Accrual paused for this car so half‑built configs can't poison it.</div></div><button class="lab-mode" data-clone-unlock="1">🔍 Unlock · follow live</button></div>`;
      }
    ...
    const trackRecordHtml = (tr, rn) => { const carLabel = (c) => `${esc(c.name || "#" + String(c.cid).split("|")[0])} ${piBadge(c.class || null, c.pi || null, true)}`.trim(); return tr ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--accent2);border-radius:8px;background:var(--bg2)">
...
${open.length ? `<div style="font-size:11px;color:var(--warn,#e3b341);margin-top:4px">🔍 ${open.map((a) => a.text).join(" · ")}</div>` : ""}</div>`; }).join("")}`;
    ...
    function frameTiles(f) {
      return [[f.mph.toFixed(0), "mph"], [f.gear === 0 ? "R/N" : f.gear === 11 ? "⇅" : f.gear, "gear"], [f.rpm, "rpm"], [f.lat.toFixed(2), "lat g"], [f.lon.toFixed(2), "long g"], [f.yaw.toFixed(0), "yaw °/s"], [f.hp, "hp"], [f.boost.toFixed(1), "boost psi"], [f.on ? piBadge(f.cls, f.pi, true) : "—", f.on ? `${(NAMES()[String(f.car)] || {}).name || "#" + f.car} · ${f.drv} ${f.cyl || ""}cyl` : "not driving"], [f.on ? (f.ev ? `EVENT${f.lapn ? " · lap " + f.lapn : ""}${f.rpos ? " · P" + f.rpos : ""}` : "free roam") : "—", f.on && f.ev ? `${(f.dist / 1000).toFixed(2)} km · ${f.lapt ? f.lapt.toFixed(1) + " s" : ""}` : "mode"]];
    }
    ...
    function paintFrame() {
      ...
      if (wl) {
        ...
        if (wf === "course") {
          ...
          html = `<span class="chip" style="border-color:var(--accent);color:var(--accent);font-weight:700">🛣 FREE TUNING</span> <b>${esc(carNm)}</b>${f.on ? ` ${piBadge(f.cls, f.pi, true)} · ${f.drv}` : " · not driving"}${an && an.summary ? ` · ${an.summary.corners} corners · ${an.summary.launches} launches · ${an.summary.braking} stops analysed` : ""}`;
        }
