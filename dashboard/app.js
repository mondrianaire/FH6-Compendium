...
(function () {
  ...
  function buildLab() {
    ...
    const routesAtlas = (s) => {
      const models = (DB.courseModels || []).filter((m) => m.geometry && m.geometry.path && m.geometry.path.length > 4);
      ...
      return `<div class="block" style="border-color:var(--accent2)"><div class="card-row" style="margin-top:0"><h3 style="margin:0">🗺 Routes atlas — every route learned so far (${models.length}), in world coordinates</h3><span class="chip">${models.reduce((a, m) => a + (m.laps || 0), 0)} laps · ${models.reduce((a, m) => a + ((m.sessions || []).length), 0)} session-visits</span></div>
        ...
        </svg><div style="font-size:10.5px;max-width:260px"><div><span style="color:var(--accent)">━</span> courses in this ${arr(s, "courses").length ? "session" : "view"} · <span style="color:var(--accent2)">━</span> other learned routes · ● start</div><div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:3px">${models.map((m) => `<span class="chip" style="${cur.has(m.route_key) ? "border-color:var(--accent);color:var(--accent)" : ""}" title="${m.route_key}">${esc(lbl(m)).slice(0, 26)} · ${(m.geometry.turns || []).length}T · ${m.laps || 0}L</span>`).join("")}</div><p class="why" style="font-size:10px;margin:6px 0 0">paths are learned from your own laps (reference lap per route); name a route once in the events table and the label updates here</p></div></div></div>`;
    };
    function courseSection(s, isLive) {
      ...
      return `${routesAtlas(s)}${courses.length ? `<div class="block" style="border-color:var(--accent2)"><h3 style="margin-top:0">🏟 Course — per route: what it demands, every turn quantified & classified, lap deltas per run, course-weighted suggestions${isLive ? ` <span class="chip">updates every ~20 s of driving</span>` : ""}</h3><div class="card-grid">${courses.map((co) => courseBlock(co, s)).join("")}</div></div>`
    ...
    const courseMap = (geo, corners) => {
      ...
      const pts = geo.path, lp = geo.last_path || [], layout = geo.layout_paths || []; const all = pts.concat(lp, ...layout.map((l) => l.pts));
      ...
      return `<svg viewBox="0 0 ${W} ${H}" class="tz-svg" style="max-width:${W}px;background:var(--bg);border-radius:8px">
        ...
        ${(geo.paths || [pts]).map((pc) => `<polyline fill="none" stroke="var(--accent2)" stroke-width="2.5" points="${poly(pc)}"/>`).join("")}
    ...
    const courseBlock = (co, s) => {
      ...
      return `<div class="lab-corner" style="border-left:4px solid var(--accent2)">
        ...
        ${profileCard}${co.geometry ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--bg2)">
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-start">${courseMap(co.geometry, co.corners)}<div style="font-size:10.5px;min-width:150px;max-width:320px"><div><span style="color:var(--accent2)">━</span> course path (reference lap) · <span style="color:var(--warn,#e3b341)">╌</span> your latest lap · <span style="color:var(--muted)">─</span> every recorded lap / attempt${(co.geometry.layout_paths || []).length ? ` (${co.geometry.layout_paths.length}${new Set(co.geometry.layout_paths.map((l) => l.session)).size > 1 ? ", " + new Set(co.geometry.layout_paths.map((l) => l.session)).size + " sessions" : ""}) — the spread of lines is the course layout` : ""} · <span style="color:#e5414e">●</span> turn loaded in telemetry · ○ mapped but not loaded</div><div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:3px">${(co.geometry.turns || []).map((g) => `<span class="chip" title="${g.dir} · r≈${g.radius_m} m · ${g.len_m} m long · ${g.deg != null ? g.deg + "° of heading change" : ""}">${g.id} ${g.dir === "L" ? "⬅" : "➡"} r${g.radius_m}${g.deg != null ? " · " + g.deg + "°" : ""}</span>`).join("")}</div>${(co.geometry.not_driven || []).length ? `<p class="why" style="font-size:10px;margin:5px 0 0">${co.geometry.not_driven.join(", ")}: mapped turns your telemetry never loaded — taken flat or cut; they still count as turns of the course</p>` : ""}</div></div>
        </div>` : ""}${turnsCard}
