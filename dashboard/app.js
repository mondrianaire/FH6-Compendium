  ...
  function buildLab() {
    ...
    function decodeSection(s, isLive) {
      ...
      return `
        ...
        ${(() => { const done = !!(D.decode && D.decode.pct >= 1); const lbl = roleStint("donor") ? "🎯 DONOR · " : "donor (picked) · ";
    ...
    const courseMap = (geo, corners) => {
      if (!geo || !geo.path || geo.path.length < 5) return "";
      const pts = geo.path, lp = geo.last_path || []; const all = pts.concat(lp);
      ...
      return `<svg viewBox="0 0 ${W} ${H}" class="tz-svg" style="max-width:${W}px;background:var(--bg);border-radius:8px">
        ${lp.length ? `<polyline fill="none" stroke="var(--warn,#e3b341)" stroke-width="2" stroke-dasharray="4 3" opacity=".9" points="${poly(lp)}"/>` : ""}
        <polyline fill="none" stroke="var(--accent2)" stroke-width="2.5" points="${poly(pts)}"/>
    ...
    const courseBlock = (co, s) => {
      ...
      return `<div class="lab-corner" style="border-left:4px solid var(--accent2)">
        ...
        ${profileCard}${co.geometry ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--bg2)">
          <div class="card-row" style="margin-top:0"><strong style="font-size:12px">🗺 Course map — learned from coordinates (${co.geometry.length_m} m · ${(co.geometry.turns || []).length} turns mapped${co.geometry.driven != null ? ` · ${co.geometry.driven} loaded in telemetry` : ""})</strong><span class="chip">ref lap ${(co.geometry.ref_lap || {}).lap || "—"}${co.geometry.last_lap ? ` · latest lap ${co.geometry.last_lap.lap} overlaid` : ""}</span></div>
          <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:flex-start">${courseMap(co.geometry, co.corners)}<div style="font-size:10.5px;min-width:150px;max-width:320px"><div><span style="color:var(--accent2)">━</span> course path (reference lap) · <span style="color:var(--warn,#e3b341)">╌</span> your latest lap · <span style="color:#e5414e">●</span> turn loaded in telemetry · ○ mapped but not loaded</div><div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:3px">${(co.geometry.turns || []).map((g) => `<span class="chip" title="${g.dir} · r≈${g.radius_m} m · ${g.len_m} m long · ${g.deg != null ? g.deg + "° of heading change" : ""}">${g.id} ${g.dir === "L" ? "⬅" : "➡"} r${g.radius_m}${g.deg != null ? " · " + g.deg + "°" : ""}</span>`).join("")}</div>${(co.geometry.not_driven || []).length ? `<p class="why" style="font-size:10px;margin:5px 0 0">${co.geometry.not_driven.join(", ")}: mapped turns your telemetry never loaded — taken flat or cut; they still count as turns of the course</p>` : ""}</div></div>
        </div>` : ""}${turnsCard}
