...
(function () {
  ...
  function buildLab() {
    ...
    const mapCardHtml = (geo, corners) => (geo ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--line);border-radius:8px;background:var(--bg2)">
          <div class="card-row" style="margin-top:0"><strong style="font-size:12px">🗺 Course map — learned from coordinates (${geo.length_m} m · ${(geo.turns || []).length} turns mapped${geo.driven != null ? ` · ${geo.driven} loaded in telemetry` : ""})</strong><span class="chip">${geo.from_model ? "best map on record" : "ref lap " + ((geo.ref_lap || {}).lap || "—")}${geo.last_lap ? ` · latest lap ${geo.last_lap.lap} overlaid` : ""}</span></div>
