...
(function () {
  ...
  // CONTACT. In a timed/Rivals run an impact does not merely cost time — it INVALIDATES the lap. The trace points
  // already carry it (grip code 4 = GRIP.impact.n), each with its own x,z, so the map and the speed trace can show
  ...
  const impactMarks = (pts, minD) => { try {
    const d2 = (minD || 12) ** 2, out = [];
    for (const p of pts || []) { if (!p || p[2] !== IMPACT_N || p.length < 5) continue;
      const last = out[out.length - 1];
      if (last && (last[3] - p[3]) ** 2 + (last[4] - p[4]) ** 2 <= d2) continue;   // same collision, next sample
      out.push(p); }
    return out;
  } catch (e) { return []; } };
  const impactCount = (pts) => impactMarks(pts).length;
  ...
  function buildLab() {
    ...
    // WHICH COURSE IS THE CAR ON? Every live corner was matched against (an.courses || [])[0] -- the session's
    // most recently driven course -- so in a session that touched two courses, corners on one were resolved
    // against the other's turn table. Jett: "not all courses are independent. the colossus is a giant course
    // that involves segments that may be in other courses as well", and a session can hold the Colossus and a
    // course whose road is part of it, with corners 6.6 km apart claimed by the wrong one.
    // The daemon already answers this authoritatively: it binds the loop at the START/FINISH LINE the car
    // crossed and emits its name, which for an unnamed route IS the route_key. That is the same anchor the
    // analyzer keys courses by, so the client should not be guessing from ordering. Position is the fallback
    // for a session with no loop bound, and courses[0] the last resort so this can never resolve to nothing.
    const liveCourse = () => {
      const an = live.analysis; const cs = (an && an.courses) || [];
      if (cs.length <= 1) return cs[0] || null;
      const lo = live.loop && live.loop.name;
      if (lo) { const hit = cs.find((c) => c.route_key === lo || c.name === lo); if (hit) return hit; }
      const f = live.frame;
      if (f && f.x != null && f.z != null) {
        let best = null, bd = Infinity;
        cs.forEach((c) => { const g = courseGeoFor(c); const pth = (g && (g.path || (g.paths || [])[0])) || [];
          for (let i = 0; i < pth.length; i += 4) { const d = (pth[i][0] - f.x) ** 2 + (pth[i][1] - f.z) ** 2; if (d < bd) { bd = d; best = c; } } });
        if (best) return best;
      }
      return cs[0] || null;
    };
    // A LAP TIME RENDERED SAFELY. The dashboard is fed by a live stream as well as the bundle, and a value that
    // is not a number must not take the whole page down: analyze_session wrote a geometry turn id into
    // course.best_lap on 6 of 91 session-courses (one `best` variable serving two purposes in one scope), and
    // every repaint threw "co.best_lap.toFixed is not a function" from inside the SSE handler. The analyzer no
    // longer does it and the written records are repaired, but the client should never have been one bad field
    // away from blank. Returns the em dash for anything that is not a finite number.
    const secs = (v, dp) => (typeof v === "number" && isFinite(v) ? v.toFixed(dp == null ? 3 : dp) : "—");
    const cornerKey = (c) => {
      if (!c || !c.apex || c.apex[0] == null || c.apex[1] == null) return null;
      // KEYED BY COURSE AS WELL AS ORDINAL. "ct8" alone collides: drive the Colossus and then a course whose
      // road is part of it in one session, and turn #8 of the second overwrites the personal best of turn #8 of
      // the first -- two corners kilometres apart sharing one record, so the bar you are shown is another
      // corner's. The course key is what makes a turn ordinal mean something.
      if (c.ev !== 0) { try { const mt = matchTurn(c.apex); const co = liveCourse();
        if (mt) return "ct" + ((co && co.route_key) ? co.route_key + "#" : "") + mt.n; } catch (e) {} }   // J10: canonical turn id when the course knows its turnstable, unlike raw grid buckets. J14: a free-roam corner (ev:0) never shares a course turn's key — its pace must not become the turn's baseline
    ...
    const markMapAt = (x, z, col) => { try { host.querySelectorAll("svg[data-live-map]").forEach((sv) => {
      ...
      // NaN FIRST. Every comparison against NaN is false, so a NaN coordinate sailed through all four bounds
      // checks below, reached toFixed as the string "NaN", and produced an invalid transform — which SVG treats
      // as no transform at all and paints at the origin. That is the marker snapping to the TOP-LEFT and then
      // sliding back on the next good point. A missing dataset attr on any matched map (+undefined) is enough.
      if (!isFinite(cx) || !isFinite(cy)) { const gp = sv.querySelector(".spd-mark"); if (gp) gp.style.display = "none"; return; }
    ...
    const trackIndex = (s) => {
      ...
      const rows = models.slice().sort((a, b) => (b.laps || 0) - (a.laps || 0)).map((m) => { const bl = Object.entries(m.best_laps || {}).map(([cid, v]) => Object.assign({ cid }, v)).sort((a, b) => a.best_lap - b.best_lap); const best = bl[0]; const g = m.geometry || {}; const visits = m.visits || []; // A ROUTE YOU STARTED AND NEVER FINISHED IS NOT A COURSE YET. Jett: "I have a habit of starting very long
        // courses but never finishing them, could these just be legs of a run that didnt finish?" — they are, and
        // presenting them beside real courses is most of why the catalog reads as clutter. The test is not length
        // or lap count: it is that NO lap was ever timed here and the road does not close, i.e. nothing has ever
        // gone start-to-finish. Six of 21 courses are this today, including a 47.6 km one with 8 lap records.
        // Kept, never deleted — it is real driving, and the day a lap completes it becomes a course by itself.
        const _anyTimed = Object.values(m.best_laps || {}).some((v) => v && v.best_lap);
        const _gp = (m.geometry || {}).path || [];
        const _closes = _gp.length > 2 && Math.hypot(_gp[0][0] - _gp[_gp.length - 1][0], _gp[0][1] - _gp[_gp.length - 1][1]) <= Math.max(60, 0.12 * ((m.geometry || {}).length_m || 0));
        // STATE THE FACT, NOT THE ACCUSATION. The first cut called this "never finished" and immediately flagged
        // Hakone Nanamagari — a named touge with 13 laps on it. A road you drive for pleasure has no timed lap and
        // does not close, and neither makes it an abandoned run. What is actually true, and all that is true, is
        // that nothing here has ever been timed: no reference lap, so no lap deltas and no 107% comparison.
        // Whether that is an abandoned Colossus attempt or a mountain road you enjoy is the driver's business.
        const unfinished = !_anyTimed && !_closes;
        const kind = m.route_key.startsWith("loop:") ? "reference loop" : unfinished ? "no timed lap" : (visits.some((v) => (v.laps || 0) > (v.attempts || 0)) ? "circuit" : "point-to-point"); const sel = atlasPick === m.route_key;
        return `<tr data-atlas-route="${esc(m.route_key)}" title="${unfinished ? esc("no lap has ever been timed here — so no reference lap, no deltas, no 107% comparison. Either an unfinished run, or a road you drive rather than race.") : ""}" style="cursor:pointer;${unfinished ? "opacity:.55;" : ""}${sel ? "outline:2px solid var(--accent);outline-offset:-2px" : cur.has(m.route_key) ? "background:rgba(0,210,122,.06)" : ""}"><td>${courseIdentMini(lbl(m), g, m.route_key, 22)}${cur.has(m.route_key) ? ` <span class="chip" style="border-color:var(--accent);color:var(--accent)">in this recording</span>` : ""}</td><td>${kind}${(() => { const ct = courseTags(m); return ct && (ct.tags.length || ct.fits.length) ? `<div style="display:flex;flex-wrap:wrap;gap:3px;margin-top:2px">${ct.tags.slice(0, 2).concat(ct.fits.slice(0, 1)).map((t2) => `<span class="chip" style="border-color:${t2.col};color:${t2.col};font-size:9.5px;padding:1px 5px" title="${esc(t2.why)}">${t2.t}</span>`).join("")}</div>` : ""; })()}</td><td>${g.length_m ? (g.length_m / 1000).toFixed(2) + " km" : "—"}</td><td>${(g.turns || []).length || "—"}</td><td>${m.laps || 0}</td><td>${visits.reduce((a, v) => a + (v.attempts || 0), 0)}</td><td>${(m.sessions || []).length}</td><td>${best ? `<b style="color:#00d27a">${secs(best.best_lap)} s</b>` : "—"}</td><td>${best ? `${buildThumb(String(best.cid).split("|")[0], best.build_id, true)} ${esc(best.name || best.cid)} <span class="why"${best.build_id ? ` title="build fingerprint ${esc(String(best.build_id))}"` : ""}>${piBadge(best.class || null, best.pi || null, true)}${best.drivetrain ? " " + best.drivetrain : ""}${best.hp ? " · " + best.hp + " hp" : ""}</span>` : "—"}</td><td class="why">${(m.sessions || []).slice(-1)[0] ? String((m.sessions || []).slice(-1)[0]).slice(-6) : ""}</td></tr>`; }).join("");
    ...
    // buildRecordFor lived here: defined once, called nowhere, and reading DB.builds — a key db.js has never
    // had (29 top-level keys, none of them `builds`). Its window.__builds fallback had no writer either.
    ...
    const canonTurns = () => { const co = liveCourse(); return (co ? turnTable(co).turns : []); };   // legacy shim: live course, model-first
    const matchTurn = (apex) => { if (!apex || apex[0] == null) return null; const co = liveCourse(); const t = co && turnAt(co, apex); return t ? { id: t.id, n: t.n, dir: t.dir } : null; };
    ...
    function decodeSection(s, isLive) {
      ...
      return `${subject}${libraryBlock}
        <div class="block" style="border-color:#e3b341"><h3 style="margin-top:0">🧬 Decode — clone a build: donor capture → Clone Sheet (the parts) → Bench convergence (the sliders)</h3>
          <p class="why" style="font-size:12px">${pinNote ? pinNote + " · " : ""}${hasRoles || pin ? "Donor: " + (pin ? "📌 pinned — stays until you pick another donor or unpin it" : "from the run roles (🎯 / 🔧)") + "." : isLive ? "Set <b>🎯 DONOR</b> on the locked-tune run and <b>🔧 REPLICA</b> on your rebuild with the 🎯 / 🔧 buttons just above — or pick runs below." : "No run roles in this recording — pick the donor run and the replica run below."} ${sameRun ? "Same run on both sides: a self-check until a replica run exists." : diffCar ? "<b>Different cars on the two sides</b> — a demo comparison; the ledger will be honestly red." : ""}${isLive && pin ? ` <button class="lab-mode" data-unpin style="padding:2px 8px;font-size:11px;margin-left:6px">✕ unpin donor</button>` : ""}</p>
          <div class="lab-bench">
    ...
    const courseMap = (geo, corners, turns, opts) => {
      ...
      // SHAPE has to carry the meaning. pointer-events:none so it can never swallow a turn marker's click.
      const impactPath = (() => { try {
    ...
    const courseMeasuredChip = (x) => { try {
      ...
      return `<span class="chip" style="border-color:#00d27a;color:#00d27a" title="measured, not guessed — the fastest recorded lap here: ${esc(b.name || b.cid || "")}${b.hp ? " · " + b.hp + " hp" : ""}">🏆 fastest here: ${buildThumb(String(b.cid || "").split("|")[0], b.build_id, true)} ${piBadge(b.class || null, b.pi || null, true)}${b.drivetrain ? " " + esc(b.drivetrain) : ""} · ${secs(b.best_lap)} s</span>`;
    } catch (e) { return ""; } };
    ...
    const speedTracesCard = (co, curCar) => { try {
      ...
      const impLap = cur || best || match[0];
      const impTicks = (() => { try {
        const hits = impactMarks((impLap && impLap.pts) || []); if (!hits.length) return "";
        const vd = isVoid(impLap);
        return `<g class="spd-imp" pointer-events="none" stroke="${gripCol("impact")}" fill="${gripCol("impact")}">${hits.map((p, i) => { const x2 = px2(p[0]).toFixed(1), y2 = py2(p[1]).toFixed(1);
          return `<g><title>impact ${i + 1} of ${hits.length} at ${Math.round(p[0])} m${vd ? " — " + esc(VOID_WHY) : ""}</title><line x1="${x2}" y1="6" x2="${x2}" y2="${H2 - padB}" stroke-width="1" opacity=".55" stroke-dasharray="2 2"/><path transform="translate(${x2},${y2})" d="M 0 -5.5 L 1.5 -1.5 L 5.5 0 L 1.5 1.5 L 0 5.5 L -1.5 1.5 L -5.5 0 L -1.5 -1.5 Z" stroke="var(--bg)" stroke-width="0.9"/></g>`; }).join("")}</g>`;
      } catch (e) { return ""; } })();
    ...
    const trackRecordHtml = (tr, rn, trGeo, trRk) => { const carLabel = (c) => `${buildThumb(String(c.cid).split("|")[0], c.build_id, true)} ${esc(c.name || "#" + String(c.cid).split("|")[0])} ${piBadge(c.class || null, c.pi || null, true)}`.trim(); return tr ? `<div style="margin:8px 0;padding:8px 10px;border:1px solid var(--accent2);border-radius:8px;background:var(--bg2)">
          ...
          <div style="font-size:11.5px;margin-top:4px">${tr.best ? `track best <b style="color:#00d27a">${secs(tr.best.best_lap)} s</b> <span class="why">(${carLabel(tr.best)}${tr.best.session ? ", " + tr.best.session.slice(-6) : ""})</span>` : "no timed lap yet"}${tr.this_session_best ? ` · this session <b>${tr.this_session_best.toFixed(3)} s</b>${tr.delta_to_track_best != null ? ` <span style="color:${tr.delta_to_track_best <= 0 ? "#00d27a" : "var(--warn,#e3b341)"}">(${tr.delta_to_track_best > 0 ? "+" : ""}${tr.delta_to_track_best.toFixed(3)})</span>` : ""}` : ""}</div>
          ${(tr.best_laps || []).length > 1 || ((tr.best_laps || []).length === 1 && (tr.cars || []).length > 1) ? `<div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:4px">${tr.best_laps.map((b, i) => `<span class="chip" title="session ${b.session}" style="${i === 0 ? "border-color:#00d27a;color:#00d27a" : ""}">${carLabel(b)} · ${secs(b.best_lap)} s</span>`).join("")}</div>` : ""}
          ${(tr.visits || []).length > 1 ? `<div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:3px;font-size:10.5px"><span class="why">visits:</span>${tr.visits.map((v) => `<span class="chip" title="${v.session}${v.best_car ? " · " + v.best_car : ""}">${v.session.slice(-6)} · ${v.laps}L${v.best_lap ? " · " + secs(v.best_lap, 2) : ""}</span>`).join("")}</div>` : ""}
    ...
    const courseParts = (co, s) => {
      ...
      const confChip = `<span class="chip" style="border-color:${cov.overall >= 0.8 ? "#00d27a" : cov.overall >= 0.45 ? "#e3b341" : "#e5414e"};color:${cov.overall >= 0.8 ? "#00d27a" : cov.overall >= 0.45 ? "#e3b341" : "#e5414e"}">course confidence ${Math.round(cov.overall * 100)}% · ${nl} lap${nl === 1 ? "" : "s"}${co.best_lap ? " · best " + secs(co.best_lap) + " s" : ""}</span>`;
...
${open.length ? `<div style="font-size:11px;color:var(--warn,#e3b341);margin-top:4px">🔍 ${open.map((a) => a.text).join(" · ")}</div>` : ""}</div>`; }).join("")}`;
    ...
    const dockDataState = () => {
      ...
      let courseNeeds = [];
      // the course you are ON, not the session's most recent -- liveSess() may be the FULL session (geometry
      // intact) while liveCourse() reads the stripped live one, so match across by route_key rather than index.
      if (effMode() === "course") { const ls = liveSess(); const lc = liveCourse();
        const co = (ls && lc && (ls.courses || []).find((c) => c.route_key === lc.route_key)) || (ls && (ls.courses || [])[0]);
