...
(function () {
  ...
  function buildLab() {
    ...
    const courseKnowledge = (co) => {
      ...
      const mapped = !!(geo && (geo.turns || []).length); const turnConf = tu.track_confidence || tu.confidence || 0; const poss = tu.possible || 0; const notDriven = (geo && geo.not_driven) || []; const prof = !!co.profile;
    ...
    const courseStageBanner = (co, ck) => {
      const training = ck.stage === "training"; const top = ck.needs[0];
      return `<div class="lab-corner" style="border-left:4px solid ${training ? "var(--accent2)" : "var(--accent)"};background:var(--bg2);margin-bottom:8px">
        <div class="card-row" style="margin-top:0"><strong style="font-size:15px">${training ? "📚 COURSE TRAINING" : "🏋 COURSE TUNING"} <span class="why" style="font-weight:400">· course knowledge ${ck.pct}%${training ? " — learning THIS COURSE first (car-independent)" : " — the course is known; feedback is about this car on it"}</span></strong><span style="display:inline-flex;align-items:center;gap:4px"><span id="lvCourseLive" class="chip"></span>${stageChips(ck)}</span></div>
        <div class="lab-bar" style="height:8px;margin:6px 0"><i style="width:${ck.pct}%;background:${training ? "var(--accent2)" : "var(--accent)"}"></i><i style="left:75%;width:2px;background:var(--txt);opacity:.6" title="75% — switches to tuning"></i></div>
        ${training ? `<div style="font-size:13px;margin:4px 0 2px"><b>▶ NEXT for this course:</b> ${top ? esc(top.text) : "keep lapping — the next analysis will confirm"}</div>
          <ol style="margin:4px 0 0 18px;padding:0;font-size:11.5px">${ck.needs.slice(1).map((n) => `<li>${esc(n.text)}</li>`).join("")}</ol>
          <p class="why" style="font-size:10.5px;margin:5px 0 0">what counts: full laps (map · profile) · laps (turn count earned) · every mapped turn loaded at least once · consistent lines (possible turns confirmed). Car-specific references are gathered in the background and become feedback once the course is known.</p>`
          : `<p class="why" style="font-size:11px;margin:4px 0 0">${ck.needs.length ? "the course is known; still useful for the course itself: " + ck.needs.map((n) => esc(n.text)).join(" · ") : "nothing more needed for the course — everything below is feedback for this car on it"}</p>`}
      </div>`;
    };
    const liveCourseDashboard = (co, s) => {
      ...
      return `${courseStageBanner(co, ck)}<div class="lab-tiles" style="margin-bottom:8px">${tiles.map(([v, l]) => `<div class="lab-tile"><b>${v}</b><span>${l}</span></div>`).join("")}</div>
        <div class="card-grid">${training ? `${learnPanel}<details class="lab-corner" style="border-left:4px solid var(--muted)"><summary style="cursor:pointer;font-size:12px"><b>🏋 Tuning feedback</b> <span class="chip" style="border-color:var(--warn,#e3b341);color:var(--warn,#e3b341)">locked until the course is known (≥ 75%)</span> <span class="why">— gathered in the background; open to peek</span></summary><div style="margin-top:8px">${feedPanel}</div></details>` : `${feedPanel}<details class="lab-corner" style="border-left:4px solid var(--accent2)"><summary style="cursor:pointer;font-size:12px"><b>📚 Course learning</b> <span class="chip" style="border-color:var(--accent2);color:var(--accent2)">${ck.pct}%</span> <span class="why">— known course; open for the record, map and turns</span></summary><div style="margin-top:8px">${learnPanel}</div></details>`}</div>`;
    };
    ...
    function courseSection(s, isLive) {
      ...
      if (isLive) return `${courses.length ? courses.map((co) => `<div class="block" style="border-color:var(--accent2)"><h3 style="margin-top:0">🏟 Course dashboard <span class="chip">updates every ~20 s of driving</span></h3>${selBar}${liveCourseDashboard(co, s)}</div>`).join("") : atlasPick && selModel ? `<div class="block" style="border-color:var(--accent)"><h3 style="margin-top:0">🏟 Course — from the track record</h3>${selBar}<div class="card-grid">${modelCourseCard(selModel)}</div></div>` : `<div class="block" style="border-color:var(--accent2)"><h3 style="margin-top:0">🏟 Course dashboard</h3><div class="lab-corner" style="border-left:4px solid var(--accent2);background:var(--bg2)"><div class="card-row" style="margin-top:0"><strong style="font-size:15px">📚 COURSE TRAINING <span class="why" style="font-weight:400">· no course identified yet</span></strong><span id="lvCourseLive" class="chip"></span></div><div style="font-size:13px;margin:4px 0 2px"><b>▶ NEXT for the course:</b> complete the first attempt / loop lap — the course appears (and is matched against the database) when it finishes</div><p class="why" style="font-size:10.5px;margin:5px 0 0">timed event (Rivals · race · time trial) or a marked reference loop (📍 in the stream bar). Everything outside those windows is Free Tuning.</p></div></div>`}${eventsTable(s)}${routesAtlas(s)}`;
