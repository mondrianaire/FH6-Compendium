...
### 10.5 Engine sub-menus, NSX-R (2026-09-02)

Every engine family on this car sells three tiles (Stock, Sport, Race) except Intercooler (four). Name bars read verbatim: Stock Intake, Race Fuel System, Race Ignition, Race Valves, Race Oil / Cooling, Race Flywheel, Race Intercooler, Race Cams and Valves, Race Engine Block, Race Exhaust, Race Pistons / Compression, Race Centrifugal Supercharger; all equal the catalogue base row plus tier, so the base+tier rule now holds for every engine family. Installed tiles line up with the save ids on the global ladder (cams tile 2 = index 2, pistons/block/exhaust/supercharger tile 3 = index 3): the Street tier is absent, not renumbered. Engine menus show a sixth panel, "Performance", a torque (ft-lb) and power (hp) dyno chart over RPM (x1000) that previews the highlighted part; this is the live engine preview the telemetry daemon reacts to. Power chips on the top tiles: fuel +20 hp, ignition +16, valves +15, oil +9, intercooler +26, cams +34 (from Sport); flywheel -4 lb.

Contextual dependency, restated with the 2026-09-02 counts: the Engine category shows 8 sub-menus with no aspiration conversion and 12 with the centrifugal supercharger fitted (Centrifugal Supercharger and Intercooler appear; a turbo conversion would show Turbo tiles instead). The tier bought in the Engine menu is stored in the slot named after the aspiration type, so the supercharger sub-menu and its contents exist only while that conversion is installed.
...
Drivetrain partset caveat (audit critic, 2026-09-02): drivetrain slots share one partset per container; 2102 = swapped drivetrains (313 containers), stock drivetrains use a per-car set (94 sets, 238 containers). All Drivetrain ladders proven here are on set 2102 (the AWD-swapped NSX-R). The dashboard clone sheet (fh6_tune_decode.tune_to_deliverable) still carried the superseded 2026-08-29 compound names; data/tire-compounds.json is now rewritten from the proven rows.
